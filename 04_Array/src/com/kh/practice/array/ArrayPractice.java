...
public class ArrayPractice {
	...
	public void practice2() {
		...
		for(int i=arr.length-1; i>=0; i--) {	// ** 배열이라서 무조건 0번 인덱스부터 들어가야 하니까!
			arr[i] = i+1;
			System.out.print(arr[i] + " ");
		}
	...
	public void practice5() {					//다시 생각
		...
		char[] arr = new char[str.length()];	// ** char 자료형으로 방만들기! 문자 하나씩 들어갈거니까
		
		for(int i=0; i<arr.length; i++) {	 	// 한글자씩 대입
			arr[i] = str.charAt(i);			
		}
		
		System.out.print(str + "에 " + ch + "가 존재하는 위치(인덱스) : "); // ** 따로 끊어서 출력! for문 밖에
		int count = 0;
		for(int i=0; i<arr.length; i++) {	 
			if(ch == str.charAt(i)) {			// if(ch == arr[i]) 도 가능
				System.out.print(i + " ");
				count++;
			}
		}
	...
	public void practice7() {			// 다시생각
		...
		for(int i=0; i<arr.length; i++) {
			...
			arr[i] = sc.nextInt(); 		// ** 사용자에게 입력받은 값 곧바로 인덱스에 할당!
			*/
			sum += arr[i];
		}
	...
	public void practice8() {					// 갱장쓰 어렵쓰..! 나중에 다시 생각해보자
	...
	public void practice9() {				// 다시 생각!!
		...
		for(int i=0; i<arr.length; i++) {
			if(str.equals(arr[i])) {
				System.out.println(arr[i] + "치킨 배달 가능");	// arr[i] : 각 인덱스 안에 담긴 내용물 / i : 숫자 / arr : 주소값
