...
public class PolyRun {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<arr.length; i++) {
			...
			// arr[i]의 실질적인 타입이 Child1으로 만들어졌다면 
			// Child1로 형변환하여 Child1 메소드를 호출하라.
			// 그렇지 않으면 Child2로 형변환하여 Child2 메소드를 호출하라.
		
		Parent pp = new Child1();
		pp.print();
			// Parent 타입으로 만들었을때는 얼핏보면 부모것을 실행하는것같지만
			// 실질적으로 오버라이딩되어있는지를 실행 확인을 하여
			// 오버라이딩이 되어있다면 자식에 있는 그 메소드를 실행한다 (동적바인딩)
