...
public class ElectronicRun {
	...
	public static void main(String[] args) {
		...
		// es에서 마련해 놓은 desk, note, tab 이라는 필드에
		// 객체를 생성해서 넣어주기 위해서 우선 es 생성! => 몇개?? 필드값만큼 (3개)
		...
		// 아직은 없는 메소드
		// 추가용 메소드 => insert
		// es의 insert를 호출하면 '내가 원하는 객체' 를 만들어줘! 
		// '데스크탑' 이라는 객체를 생성해서 넘기는 메소드 
		// (호출할때 new Desktop 어쩌구를 같이 들고감)
		...
		// shop1에서 던져준 desk를 자료형 맞게 조회 
		// 날아온 Desktop 객체를 (자료형 Desktop인) d 에 저장
		...
		/* 방법1
		Desktop d = (Desktop)es.select(0);	// es.select(0); 만 하면 electronic형을 뱉으니까 (자식형)으로 강제형변환 시켜준다
		NoteBook n = (NoteBook)es.select(1);
		Tablet t = (Tablet)es.select(2);
		// 이런걸 강제형변환, 다운캐스팅 이라고 한다!
		*/
		 ...
		 * 2. 메소드 정의시 매개변수로 다형성을 적용하게되면 메소드 개소가 확 줄어든다! (any 로 받아)
