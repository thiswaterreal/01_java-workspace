	...
	public static void printBooks(ArrayList<Book> bs) {
		...
		while (flag == true) { // 메뉴 스위치가 켜져있는 동안 무한반복
			...
			case 2: // 2. 추가하기
				...
				sc.nextLine(); //** [ int menu = sc.nextInt(); // 메뉴 번호 입력 ] <= 여기서 오는 엔터제거!!
				...
				if(result == 0) {	// 중복x => 추가가능
					if (bkOrMaga.equals(true)) {	// 일반도서(true)
						bc.addBook(new Book(bNo, title, author, publisher, price, description)); //** 새로운 Book객체 추가
					} else if (bkOrMaga.equals(false)) {	// 잡지(false)
						System.out.print("출간연도를 입력하세요 : ");
						int year = sc.nextInt();
						System.out.print("출간월을 입력하세요 : ");
						int month = sc.nextInt();
						bc.addBook(new Magazine(bNo, title, author, publisher, price, description, year, month)); //** 새로운 Magazine객체 추가
					}
					System.out.println("성공적으로 추가되었습니다.");	
				}else {		// 중복o => 추가불가능
					System.out.println("이미 소장하고있는 책입니다.");
				}
				break;
			...
			case 3:// 3. 책 찾기
				...
				if (search == 1) { // 3-1. bNo으로 책 찾기
				...
				} else if (search == 2) { // 3-2. 책 제목으로 책 찾기
					System.out.print("책 제목을 입력하세요 : ");
					title = sc.nextLine();
				...
				} else if (search == 3) { // 3-3. 출간연도로 잡지 찾기
				...
				} else if (search == 4) { // 3-4. 출판사로 책 찾기
					System.out.print("출판사를 입력하세요 : ");
					publisher = sc.nextLine();
					
					ArrayList<Book> searchPublisher = bc.searchBookByPublisher(publisher);
					for(int i=0; i<searchPublisher.size(); i++) {
						System.out.println(searchPublisher.get(i));						
					}
					
				} else if (search == 5) { // 3-5. 특정 가격 밑으로 책 찾기
					System.out.print("가격을 입력하세요 : ");
					price = sc.nextInt();
					sc.nextLine();
					...
					for(int i=0; i<searchPrice.size(); i++) {
						System.out.println(searchPrice.get(i));						
					}
			...
			case 4: // 4. 전체책 가격 합계 및 평균 조회
				...
				int avg = bc.getAvgPrice();
				
				System.out.println("전체책 가격 합계 : " + total + "원");
				System.out.println("전체책 가격 평균 : " + avg + "원");
				break;
